...
# 继承自 OnPolicyAlgorithm from stable_baselines3
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # try:
                #     n_iter = (
                #         rollout_data.observations["image"].shape[0]
                #         // self.mae_batch_size
                #     )
                # except:
                #     n_iter = (
                #         rollout_data.observations["tactile"].shape[0]
                #         // self.mae_batch_size
                #     )
                ...
                if "image" in observations and len(observations["image"].shape) == 5:
                    ...
                    observations["image"] = observations["image"].reshape(
                        (
                            observations["image"].shape[0],
                            observations["image"].shape[1],
                            observations["image"].shape[2],
                            -1,
                        )
                    )
                if (
                    "tactile" in observations
                    and len(observations["tactile"].shape) == 5
                ):
                    frame_stack = observations["tactile"].shape[1]
                    observations["tactile"] = observations["tactile"].reshape(
                        (
                            observations["tactile"].shape[0],
                            -1,
                ...
                #     else:
                #         x = vt_load(
                #             copy.deepcopy(observations), frame_stack=frame_stack
                #         )
